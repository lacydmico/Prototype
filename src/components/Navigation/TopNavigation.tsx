...
const TopNavigation: React.FC<TopNavigationProps> = ({ 
  ...
  const navItems = [
    ...
    { label: 'LIVE NEWS', href: '/live-news', active: false },
    { label: 'MY LIST', href: '/my-list', active: false },
  ];

  const activeChallenges = userProgress ? Object.values(userProgress.challenges).filter((c: any) => c.status === 'available').length : 0;
  const progressPercentage = userProgress ? Math.min((userProgress.totalPoints / userProgress.monthlyGoal) * 100, 100) : 0;

  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 bg-black/80 backdrop-blur-sm ${className}`}>
      <div className="flex items-center justify-between px-6 py-4">
          ...
          {navItems.map((item) => (
            <a
              ...
              className={`
                ...
                ${item.active 
                  ? 'text-white font-bold transform translate-y-[-1px]' 
                  : 'text-white/70 font-light hover:text-white'
                }
              `}
              style={{
                fontFamily: item.active ? '"Proxima Nova SemiBold", sans-serif' : '"Proxima Nova Regular", sans-serif',
                fontSize: '14px',
                letterSpacing: '0.5px',
              }}
            >
        ...
        {/* Right Section - Search, Trophy, Profile */}
        <div className="flex items-center space-x-6">
          ...
          {/* Challenge Trophy */}
          {userProgress && onOpenChallengeHub && (
            <button
              onClick={onOpenChallengeHub}
              className="relative text-white/70 hover:text-white transition-all duration-300 p-2"
              aria-label="Challenge Hub"
              style={{
                animation: 'strongGlow 1.5s ease-in-out infinite alternate, pulse 2s ease-in-out infinite'
              }}
            >
          ...
          <style jsx>{`
            @keyframes glow {
              from {
                filter: drop-shadow(0 0 8px rgba(255, 215, 0, 0.7)) drop-shadow(0 0 15px rgba(255, 215, 0, 0.5));
              }
              to {
                filter: drop-shadow(0 0 25px rgba(255, 215, 0, 1)) drop-shadow(0 0 35px rgba(255, 215, 0, 0.8));
              }
            }
            
            @keyframes strongGlow {
              0% {
                filter: drop-shadow(0 0 10px rgba(255, 215, 0, 0.8)) drop-shadow(0 0 20px rgba(255, 215, 0, 0.6)) drop-shadow(0 0 30px rgba(255, 215, 0, 0.4));
                transform: scale(1);
              }
              50% {
              ...
              100% {
                filter: drop-shadow(0 0 15px rgba(255, 215, 0, 0.9)) drop-shadow(0 0 30px rgba(255, 215, 0, 0.7)) drop-shadow(0 0 45px rgba(255, 215, 0, 0.5));
